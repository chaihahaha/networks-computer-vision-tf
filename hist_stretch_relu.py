...
use_model = 'hist_stretch1'
print_weight = '/hist_stretch1/a0'
...
out_image = hist_stretching_layer_relu(in_image[0:1,:,:,:], 10, 3)[0,:,:,:]
...
print([v for v in tf.trainable_variables() if v.name == print_weight + ":0"])
weight = tf.reduce_mean([v for v in tf.trainable_variables() if v.name == print_weight + ":0"])
