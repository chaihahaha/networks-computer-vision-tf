...
print([v for v in tf.trainable_variables() if v.name == "hist_stretch/fnM"])
weight = tf.reduce_mean([v for v in tf.trainable_variables() if v.name == "hist_stretch/fnM:0"])
...
train_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "hist_stretch")  # 设定需要训练的变量，通过scope指定
# print(train_vars)
G_opt = tf.train.AdamOptimizer(learning_rate=lr).minimize(G_loss, var_list=train_vars)   # var_list指定需要优化的变量

saver_sid = tf.train.Saver(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, "hist_stretch"))
